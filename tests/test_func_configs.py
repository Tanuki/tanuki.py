...
def test_load_save_config():
    ...
    func_modeler.function_configs[func_hash].teacher_models = [Llama_Bedrock_Config(model_name = random_string_2, context_length = 8192),
                             OpenAI_Config(model_name = "gpt-4-32k", context_length = 32768)] # model and its token limit]
    func_modeler._update_config_file(func_hash)
    ...
    assert isinstance(config.teacher_models[0], Llama_Bedrock_Config)
    assert config.teacher_models[1].model_name == "gpt-4-32k"
    assert isinstance(config.teacher_models[1], OpenAI_Config)


def test_default_config():
    config = FunctionConfig()
    assert config.distilled_model.model_name == ""
    assert isinstance(config.distilled_model, OpenAI_Config)
    assert config.teacher_models[0].model_name == "gpt-4"
    assert isinstance(config.teacher_models[0], OpenAI_Config)
    assert config.teacher_models[1].model_name == "gpt-4-32k"
    assert isinstance(config.teacher_models[1], OpenAI_Config)
...
def test_update_config_various():
    ...
    assert config.teacher_models[1].model_name == "gpt-4-3222k"
    assert config.teacher_models[1].context_length == 991
    assert isinstance(config.teacher_models[1], Llama_Bedrock_Config)
...
def test_update_config_teachers():
    ...
    assert isinstance(teacher_models_3[0], OpenAI_Config)
...
def test_update_finetune_config():
    finetune_response = FinetuneJob(id = "aas",
                                    status = "success", 
                                    fine_tuned_model=OpenAI_Config(model_name = "ayyoo-finetune", context_length = 32768))
    config = FunctionConfig()
