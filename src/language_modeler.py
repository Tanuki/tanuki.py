...
class LanguageModeler(object):
    ...
    def generate(self, args, kwargs, logger, function_description, model_type = "openai"):
        """
        The main generation function, given the args, kwargs, logger, function description and model type, generate a response and check if the datapoint can be saved to the finetune dataset
        """

        prompt, model, save_to_finetune, is_distilled_model = self.get_generation_case(args, kwargs, logger, function_description, model_type)
        if model_type == "openai":
            choice = self.openai_generate(model, prompt)
        
        output = LanguageModelOutput({"generated_response": choice, "suitable_for_finetuning": save_to_finetune, "distilled_model": is_distilled_model})
        return output
    ...
    def get_models(self, logger, func_hash):
        """
        Get the loggers models given the function hash
        """
        current_model, teacher_models = logger.get_models(func_hash)
        return current_model, teacher_models


    def get_generation_case(self, args, kwargs, logger, function_description, model_type):
        """
        ...
        current_model, teacher_models = self.get_models(logger, function_description.__hash__())
